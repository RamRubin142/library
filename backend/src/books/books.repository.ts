...
export class BooksRepository {
  ...
  async findOne(_id: string): Promise<BookDocument | null> {
    return this.BookModel.findOne({ _id })
      .populate('author')
      .populate({ path: 'readers', select: 'name _id serialId' })
      .exec();
  }

  async find(): Promise<BookDocument[] | null> {
